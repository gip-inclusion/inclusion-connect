...
def test_add_url_params():
    ...
    base_url = "http://testserver/test?next=/siae/search%3Fdistance%3D100%26city%3Dstrasbourg-67"

    url_test = add_url_params(base_url, {"test": "value"})
    assert (
        url_test == "http://testserver/test?next=%2Fsiae%2Fsearch%3Fdistance%3D100%26city%3Dstrasbourg-67&test=value"
    )

    url_test = add_url_params(base_url, {"mypath": "%2Fvalue%2Fpath"})

    assert url_test == (
        "http://testserver/test?next=%2Fsiae%2Fsearch%3Fdistance%3D100%26city%3Dstrasbourg-67"
        "&mypath=%252Fvalue%252Fpath"
    )

    url_test = add_url_params(base_url, {"mypath": None})

    assert url_test == "http://testserver/test?next=%2Fsiae%2Fsearch%3Fdistance%3D100%26city%3Dstrasbourg-67"

    url_test = add_url_params(base_url, {"mypath": ""})

    assert url_test == "http://testserver/test?next=%2Fsiae%2Fsearch%3Fdistance%3D100%26city%3Dstrasbourg-67&mypath="


def test_get_url_params():
    url = "http://testserver/test?next=/siae/search%3Fdistance%3D100%26city%3Dstrasbourg-67"
