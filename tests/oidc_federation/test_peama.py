...
def test_convert_existing_ic_user_with_unconfirmed_email(client, requests_mock, caplog):
    ...
    EmailAddressFactory(email="michel@pole-emploi.fr", user=user)
...
def test_block_user_from_another_federation(client, requests_mock, caplog):
    user = UserFactory(
        email="michel@pole-emploi.fr",
        first_name="old_first_name",
        last_name="old_last_name",
...
def test_block_user_from_another_federation_same_sub(client, requests_mock, caplog):
...
def test_logout(client, requests_mock, caplog):
...
@pytest.mark.django_db(transaction=True)
def test_logout_fails(client, requests_mock, caplog):
